    ...
    def forward(self, src, tgt, tgt_mask=None, src_pad_mask=None, tgt_pad_mask=None):
        ...
        for d in range(len(prev_token)):
            ...
            if type_tok =='n':
                ...
                out = torch.cat((torch.zeros((batch_size,seq_len,self.n_toks)).to(self.device),out,torch.zeros((batch_size,seq_len,self.v_toks+self.t_toks)).to(self.device)),dim=2).to(self.device)
            elif type_tok=='d':
                out = self.out3(transformer_out)
                out = torch.cat((torch.zeros((batch_size,seq_len,self.n_toks+self.d_toks)).to(self.device),out,torch.zeros((batch_size,seq_len,self.v_toks)).to(self.device)),dim=2).to(self.device)
            elif type_tok=='t':
                out = self.out4(transformer_out)
                out = torch.cat((torch.zeros((batch_size,seq_len,self.n_toks+self.d_toks+self.t_toks)).to(self.device),out),dim=2).to(self.device)
            elif type_tok=='v':
                out = self.out1(transformer_out)
                out = torch.cat((out,torch.zeros((batch_size,seq_len,self.v_toks+self.d_toks+self.t_toks)).to(self.device)),dim=2).to(self.device)
